...
function [model, info] = do_struct_m5il_solve_v0(bag_paths_pos, bag_paths_neg, W_init, config)
...
[kernel_constrait_D,u_D,sum_mean_psi_pos_bags] = compute_kernel_constrait_D(bag_paths_pos,config);
...
for i_init = 1:length(W_init)
    ...
    [energy_i,slack_neg_i,slack_pos_i,slack_div_i,balance_score_i,TPR_i,TNR_i,channel_distribute_i,subgradient_constrait_B_i,subgradient_constrait_C_i] = compute_energy(bag_paths_pos,bag_paths_neg,W_init{i_init},sum_mean_psi_pos_bags,config);
    if energy_i < energy
        W = W_init{i_init};
        energy = energy_i;
...
while true
    ...
    [energy_CCCP,slack_neg,slack_pos,slack_div,balance_score,TPR,TNR,channel_distribute,subgradient_constrait_B,subgradient_constrait_C] = ...
        compute_energy(bag_paths_pos,bag_paths_neg,W,sum_mean_psi_pos_bags,config);
...
%%
function [W_new, energy_cutting_plane_dual, energy_cutting_plane_primal, slack_neg,slack_pos,slack_div] = solve_cuttingplane( W, subgradient_constrait_B,subgradient_constrait_C, kerMat_ws, uMat_ws, bag_paths_pos,bag_paths_neg, config )
...
for s = 1: num_const_term
    ...
    switch s
        case 1
            ...
            value_vlt_new = find_most_vlt_const_neg( W_new, bag_paths_neg, config);            
        case 2
            % (2) for constraint B:
    ...
    for s = 1: num_const_term
        ...
        switch s
            case 1
                % (1) for constraint A:
                W_new = reshape(W_vec,size(W));
                [value_vlt_new, uVec_new, b_new] = find_most_vlt_const_neg( W_new, bag_paths_neg, config);                      
            case 2
                % (2) for constraint B:              
...
function [value_vlt, uVec, b] = find_most_vlt_const_neg( W, bag_paths_neg, config)      

bags_num_neg = length(bag_paths_neg);
...
if config.using_parallel
     parfor i = 1:bags_num_neg
        [scores_neg_all_channel(i,:),psis{i}] = infer_bag_score(bag_paths_neg{i},W,config);
    end
else
...
function [energy_CCCP,slack_neg,slack_pos,slack_div,balance_score,TPR,TNR,channel_distribute,subgradient_constrait_B,subgradient_constrait_C] = compute_energy(bag_paths_pos,bag_paths_neg,W,sum_mean_psi_pos_bags,config)
...
if config.using_parallel
     parfor i = 1:bags_num_neg
        scores_neg_all_channel(i,:) = infer_bag_score(bag_paths_neg{i},W,config);
    end
else
    for i = 1:bags_num_neg
        scores_neg_all_channel(i,:) = infer_bag_score(bag_paths_neg{i},W,config);
...
bags_num_pos = length(bag_paths_pos);
...
if config.using_parallel
     parfor i = 1:bags_num_pos
        [scores_pos_all_channel(i,:),psis_max_score{i},divs(i),psis_max_div{i}]= infer_bag_score_div(bag_paths_pos{i},W,config);
    end
else
...
function [scores,psi_max_score, div, psi_max_div] = infer_bag_score_div(bag_path,W,config)


if ~exist(bag_path{1},'file')
    fprintf('WRONG!!! the parts_features file not exist! %s\n',bag_path{1});
    return;
else
    load(bag_path{1},'parts_features');
end

if ~exist(bag_path{2},'file')
    fprintf('WRONG!!! the parts_pair_features file not exist! %s\n',bag_path{2});
    return;
else
    load(bag_path{2},'parts_pair_features');
end
...
function [scores,psi_max_score] = infer_bag_score(bag_path,W,config)
