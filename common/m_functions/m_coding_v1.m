...
% waring: if using FV coding, the size of codes is too large to store, so
% for FV coding, the coding is done just before using

function return_state = m_coding_v1(global_config)
return_state = 1;
        ...
        for i_feature = 1:length(feature_types)
            ...
            switch coding_config.params.method
                ...
                case 'FV'
                    ...
                    for i_vid = 1:length(cur_vid_idx)
                        do_coding_FV(cur_splits,cur_feature,cur_class,cur_vid_idx(i_vid),gmm_means,gmm_covs,gmm_priors,pca_base,pca_mean,global_config);
                    end
...
function do_coding_FV(cur_splits,cur_feature,i_class,i_vid,gmm_means,gmm_covs,gmm_priors,pca_base,pca_mean,global_config)
