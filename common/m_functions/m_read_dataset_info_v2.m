...
%v0 -> v1 add read Hollywood
%v1 -> v2 support UCF11
function return_state = m_read_dataset_info_v2(global_config)
    ...
    case 'Scene-15'
        ...
        for i = 1:length(class_folder_names)
            ...
            for j = 1:length(file_names)
                ...
                img_paths{i_class}{i_img} = fullfile(i_class_names, file_names(j).name);
                img_names{i_class}{i_img} = file_names(j).name;
            end
            
            img_nums_in_class = [img_nums_in_class; i_img];
                
        end
        
        if (i_class == 0)
            fprintf('no class exist!\n');
            return_state = 0;
            return
        end
        ...
        train_num_per_class = 100;
        test_num_per_class = -1;
        % read the split info
        splits_num = 1;
        ...
        for i_split = 1:splits_num
            ...
            for i_class = 1:class_num
                ...
                idx = [1:length(img_names{i_class})];
                sel = randsample(idx,train_num_per_class);
                train_idx = idx(sel);
                idx(sel) = [];

                if (test_num_per_class == -1 || length(idx) <= test_num_per_class)
                    test_idx = idx;
                else
                    sel2 = randsample(idx,test_num_per_class);
                    test_idx = idx(sel2);
                end
                    
                train_set_idx{i_split}{i_class} = train_idx;
                if (train_num_per_class ~= length(train_idx))
                    fprintf('get train data set split wrong at class%03d!\n',i_class);
                    return_state = 0;
                    return;
                end
                
                test_set_idx{i_split}{i_class} = test_idx;
                if (test_num_per_class == -1 && length(test_idx) ~= (length(img_names{i_class}) - length(train_idx))) ...                
                ||(test_num_per_class ~= -1 && test_num_per_class ~= length(test_idx))
        ...
        % save
        save(global_config.read_dataset_info.file_name,'img_paths','img_names','class_names','img_nums_in_class','class_num','img_total_num',...
...
%         
%         save(global_config.read_dataset_info.file_name,'vid_paths','vid_names','class_names','vid_nums_in_class','class_num','vid_total_num',...
%             'splits_num','test_set_idx','train_set_idx','test_num_per_class','train_num_per_class','train_valid','test_valid');
%         
%         
%     case 'UCF11'
%         
