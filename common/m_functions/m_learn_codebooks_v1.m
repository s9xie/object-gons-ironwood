...
function return_state = m_learn_codebooks_v1(global_config)
return_state = 1;
...
switch learn_codebooks_config.method
    
    
    
    case 'BoW' % Bag of Words method
        
        fprintf('Learning BoW codebook...\n');
    ...
    case 'FV'   % fisher vector method
         fprintf('Learning FV GMM...\n');
       
        if global_config.num_core > 1
            parfor i_param = 1:size(split_feature_type_params,1)
                do_learn_gmm(split_feature_type_params{i_param,1},split_feature_type_params{i_param,2},...
                    features_num,global_config);
            end
            
        else
            for i_param = 1:size(split_feature_type_params,1)
                if ~do_learn_gmm(split_feature_type_params{i_param,1},split_feature_type_params{i_param,2},...
                        features_num,global_config)
                    return_state = 0;
                    return;
                end
...
%%
function return_state = do_learn_gmm(cur_splits,cur_feature,features_num,global_config)
return_state = 1;
...
if (learn_codebooks_config.params.re_select_data) ||...
    ...
    for i_class = 1:class_num
        ...
        if total_dt_feature_num_curClass > learn_codebooks_config.params.FV.select_data_num_each_class
            select_idx = randperm(total_dt_feature_num_curClass,learn_codebooks_config.params.FV.select_data_num_each_class);
    ...
    if size(select_data,1) > learn_codebooks_config.params.FV.select_data_num_total
        select_data = select_data(randperm(size(select_data,1),learn_codebooks_config.params.FV.select_data_num_total),:);
    end
    
    
    save_name = fullfile(global_config.learn_codebooks.path,sprintf('FV_select_data_s%02d_%s.mat',cur_splits,cur_feature));
    save(save_name,'select_data');
else
    loadName = fullfile(global_config.learn_codebooks.path,sprintf('FV_select_data_s%02d_%s.mat',cur_splits,cur_feature));
    load(loadName);
end


pca_mean = mean(select_data,1);
select_data = select_data - repmat(pca_mean,size(select_data,1),1);

if size(select_data,2) > global_config.learn_codebooks.params.FV.not_reduced_max_dim
    % do PCA and save PCA base
    pca_base = pca(select_data);
    reduced_dim = round(global_config.learn_codebooks.params.FV.reduced_factor*size(select_data,2));
    pca_base = pca_base(:,1:reduced_dim);
    select_data = select_data*pca_base;
else
    pca_base = eye(size(select_data,2));
end

% vl_gmm
fprintf('learn gmm of split:%d feature:%s\n',cur_splits,cur_feature);
[gmm_means, gmm_covs, gmm_priors] = vl_gmm(select_data',learn_codebooks_config.params.FV.K, 'verbose','MaxNumIterations', 100);

save_name = fullfile(learn_codebooks_config.path,sprintf('gmms_s%02d_%s.mat',cur_splits,cur_feature));
save(save_name,'gmm_means','gmm_covs','gmm_priors','pca_base','pca_mean');
...
if (learn_codebooks_config.params.re_select_data) ||...
    ...
    for i_class = 1:class_num
        ...
        if total_dt_feature_num_curClass > learn_codebooks_config.params.BoW.select_data_num_each_class
            select_idx = randperm(total_dt_feature_num_curClass,learn_codebooks_config.params.BoW.select_data_num_each_class);
    ...
    if size(select_data,1) > learn_codebooks_config.params.BoW.select_data_num_total
        select_data = select_data(randperm(size(select_data,1),learn_codebooks_config.params.BoW.select_data_num_total),:);
    end
    
    
    save_name = fullfile(global_config.learn_codebooks.path,sprintf('BOW_select_data_s%02d_%s.mat',cur_splits,cur_feature));
    save(save_name,'select_data');
else
    loadName = fullfile(global_config.learn_codebooks.path,sprintf('BOW_select_data_s%02d_%s.mat',cur_splits,cur_feature));
    load(loadName);
end

% vl_kmeans
fprintf('kmeans of split:%d feature:%s\n',cur_splits,cur_feature);
codebooks = vl_kmeans(select_data',learn_codebooks_config.params.BoW.codebook_size,'numRepetitions',8,'algorithm', 'elkan')';
