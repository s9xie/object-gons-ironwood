...
function  do_clustering_v3(cur_class,cur_vid,clustering_config,feature_path)
...
%         tic;
%         eigen_vals = svds(cur_affinity_matrix+sparse(eye(length(groups{i}))));
%         eigen_vals = svds(sparse(full(cur_affinity_matrix)+eye(length(groups{i}))),max_cluster_num);
%         toc;
...
%         [~, cluster_num ] = min(sc_val);
        ...
        cur_edges_num = length(cur_edges_weight)/2;
        
        % save('debug_ganc_data.mat','cur_affinity_matrix_up','cur_edges_num','cluster_num');
        
        hirarchy_ratio = 0.6;
        for i_level = 1:3
            cur_level_cluster_num_target = round(cur_group_cluster_num_target*(hirarchy_ratio^(i_level-1)));
            ...
            % soft cluster
            cur_clusters = cell(cur_level_cluster_num,1);
            for cur_i_cluster = 1:cur_level_cluster_num
                cur_clusters{cur_i_cluster} = groups{i}(find(cur_cluster_id == cur_i_cluster));
            end
            
            cur_clusters_soft = soft_clustering(cur_clusters,location,vid_info,affinity_matrix);
            
            for cur_i_cluster = 1:cur_level_cluster_num
                
                cluster_candidate_count = cluster_candidate_count +1;
                clusters_candidate{cluster_candidate_count} = cur_clusters_soft{cur_i_cluster};
...
%%
function clusters_soft = soft_clustering(clusters,location,vid_info,affinity_matrix)
    
clusters_soft = clusters;

% local paramters
center_candidate_num = 10;
soft_cluster_num = 3;
aff_thread = 0.1;

% vid_info: [width, height, len, dt_number]
% location: [t,x,y]
% normalize
location(:,1) = location(:,1)/vid_info(3);
location(:,2) = location(:,2)/vid_info(1);
location(:,3) = location(:,3)/vid_info(2);

cluster_num = length(clusters);

center_candidate_num = min(center_candidate_num,cluster_num);
soft_cluster_num = min(soft_cluster_num,cluster_num);
cluster_center_loc = zeros(cluster_num,3);
% compute the center location of each cluster
for i_cluster = 1:cluster_num
    cur_location = location(clusters{i_cluster},1:3);
    cluster_center_loc(i_cluster,:) = mean(cur_location,1);
end

% fprintf('soft cluster\n');
% tic;

for i_cluster = 1:cluster_num

    % compute the candidate clusters
%     cur_center_loc = cluster_center_loc(i_cluster,:);
%     dis_mat = cluster_center_loc - repmat(cur_center_loc,cluster_num,1);
%     
%     dis = sum(dis_mat.^2,2);
%     [~,candidate_cluster_id] = sort(dis,'ascend');
%     
%     candidate_cluster_id = candidate_cluster_id(2:center_candidate_num);
%     
%     
%     cur_cluster_dt_id = clusters{i_cluster};    
%     candidate_cluster_aff = zeros(length(cur_cluster_dt_id),center_candidate_num);
%     
%    
%     for i = 1:center_candidate_num
% 
%         affs = affinity_matrix(cur_cluster_dt_id,clusters{candidate_cluster_id(i)});       
%         candidate_cluster_aff(:,i) = max(affs,2);
%         
%     end
    cur_cluster_dt_id = clusters{i_cluster};
    cluster_aff = zeros(length(cur_cluster_dt_id),cluster_num);
    ...
    for i = 1:cluster_num
        ...
        affs = affinity_matrix(cur_cluster_dt_id,clusters{i});       
        cluster_aff(:,i) = max(full(affs),[],2);      
    end
    
    
    [cluster_aff_sort,dt_soft_cluster_id] = sort(cluster_aff,2,'descend');
    
    dt_soft_cluster_id = dt_soft_cluster_id(:,1:soft_cluster_num);
    
    for i_soft = 1:soft_cluster_num
        
        for i_dt = 1:length(cur_cluster_dt_id)
            
            
            if (cluster_aff_sort(i_dt,i_soft) > aff_thread)
                
                clusters_soft{ dt_soft_cluster_id(i_dt,i_soft)} = [clusters_soft{ dt_soft_cluster_id(i_dt,i_soft)}; cur_cluster_dt_id(i_dt)];
