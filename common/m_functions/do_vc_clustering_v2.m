...
function  do_vc_clustering_v2(cur_class,cur_img,class_names,img_names,clustering_config,feature_path)
...
%first load the visual concept
load_name = fullfile(feature_path, class_names{cur_class}, [img_names{cur_class}{cur_img}(1:end-4)]);
load(load_name);
% 
% load_name = fullfile(feature_path, sprintf('c%03d_v%03d_trajectory',cur_class,cur_img));
% load(load_name,'trajectory');
% 
% load_name = fullfile(feature_path, sprintf('c%03d_v%03d_info',cur_class,cur_img));
% load(load_name,'vid_info');
% 
% dt_len = size(trajectory,2)/2;
% N = size(trajectory,1);
vc_len = size(response,1);
N = size(response,2);

KList = [2, 4, 8, 16];

cluster_id = cell(1,length(KList));
for id = 1: length(KList),
    [~, cluster_id{id}] = vl_kmeans(response, KList(id));
    %A{id} = reshape(A{id}, size(resps, 2), size(resps, 3));
    %paint(A, nameList{k}, vis_ori_fd, kList);
end
...
% 
% 
% fprintf('clustering c:%03d v:%03d  cluster_num:%5d \n',cur_class,cur_img,cluster_count);
% 
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % save
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

save_name = fullfile(clustering_config.path,sprintf('clusters_c%03d_v%03d.mat',cur_class,cur_img));
