...
function return_state = m_clustering_v2(global_config)
return_state = 1;
...
for i_class = 1:class_num
    ...
    if global_config.num_core > 1
        parfor i_vid = 1: length(cur_class_vids)
            do_clustering_v2(cur_class,cur_class_vids(i_vid),clustering_config,global_config.extract_features.path)
        end
    else
