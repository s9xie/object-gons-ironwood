...
function  do_clustering_v1(cur_class,cur_vid,clustering_config,feature_path)
...
for i = 1:group_num
    ...
    cluster_num = round(cur_group_node_num/N * target_cluster_num); % the parameter should be updated
    cluster_num = min(cluster_num,round(cur_group_node_num/(min_cluster_size*2)));
    
    if (cluster_num >= 2)  % a big group, at least get 2 clusters
...
%         [~, cluster_num ] = min(sc_val);
        ...
        cur_cluster_id = ganc_mex(cur_affinity_matrix,cur_edges_num,cluster_num);
